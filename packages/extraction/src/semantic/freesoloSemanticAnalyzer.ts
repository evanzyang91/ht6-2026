...
The rule field is reusable engineering knowledge inferred from all evidence, not a quotation or light paraphrase of reviewComment. Write one normative, context-complete convention that names the applicable scope or code role, states the invariant independently of this PR, resolves words such as "this", "it", and "here" using the code and file context, and preserves repository-specific helpers when the evidence establishes them. It must not mention "the reviewer", "this comment", or "this PR", and actionable rules must not end as questions. Use rationale to explain how rejected and accepted evidence supports the synthesized rule. For question-nonactionable feedback, state the unresolved engineering decision without inventing a settled convention.
...
export const ENGINEERING_MEMORY_USER_INSTRUCTION = "Normalize one pull-request review episode into the Engineering Memory SemanticAnalysis v2 schema. Use only the supplied evidence and return raw JSON without Markdown. Return exactly: intent, title, rule, rationale, detection. intent must be one of actionable-change, architecture, testing, security, style, question-nonactionable. Synthesize rule as reusable, context-complete engineering knowledge rather than copying reviewComment; resolve this/it/here from the supplied code and path. Signals must be exact code substrings, never English descriptions. detection contains mode, semanticDescription, triggerSignals, forbiddenSignals, requiredSignals, matchScope. Use forbidden-signal when code is disallowed in the trigger context, missing-required-signal when the trigger requires absent code, and semantic only when deterministic signals cannot represent the condition. Use codeContext to understand the enclosing symbol and imports, but do not invent repository facts.";
