...
export function scoreLinkageQuality(
  rejectedCode: string,
  acceptedCode?: string,
  evidence: { matchedInMergedPatch?: boolean } = {}
): LinkageQuality {
  if (!acceptedCode) return "unknown";
  const normalize = (value: string) => value.replace(/\s+/g, " ").trim();
  if (!normalize(rejectedCode) || normalize(rejectedCode) === normalize(acceptedCode)) return "unknown";
  if (evidence.matchedInMergedPatch) return "high";
  if (normalize(rejectedCode) !== normalize(acceptedCode)) return "medium";
  return "unknown";
}
